...
from sqlalchemy import Column, String, Text, Integer, Boolean, ForeignKey, JSON, DateTime, inspect, Index
...
class CompanyORM(Base):
    ...
    def to_dict(self, include_relationships: bool = False) -> dict:
        ...
        return {
            ...
            "jobs": [job.to_dict() for job in self.jobs] if hasattr(self, 'jobs') and include_relationships else [],
...
class CompanyFollowingORM(Base):
    """
        Candidates / JobSeekers can follow companies their intent to effect a follow will be recorded in this
        Model.
    """
...
class SavedCandidatesORM(Base):
    ...
    saved_by_employer = relationship("EmployerORM")
    ...
    def to_dict(self, include_internal=False, include_relationships=False):
        data = {
            ...
            "candidate": self.candidate.to_dict() if self.candidate and include_relationships else {},
            "company": self.company.to_dict() if self.company and include_relationships else {},
            "saved_by_employer": self.saved_by_employer.to_dict() if self.saved_by_employer and include_relationships else {},
        }
