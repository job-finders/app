...
from src.database.models.job_statistics import (
    ...
    TrendAnalysis, TrendDataPoint, CompanyStatistics, StatisticsError
)
...
class JobStatisticsService:
    ...
    def _initialize_redis(self):
        ...
        try:
            ...
            logger.warning(f"Redis not available for statistics caching: {e}")
            self.redis_client = None
    ...
    async def get_job_statistics(self, job_id: str) -> Optional[JobStatistics]:
        ...
        try:
            ...
            try:
                cached_stats = await self._get_cached_statistics(job_id)
            ...
            try:
                await self._cache_statistics(job_id, statistics)
            except Exception as cache_error:
    ...
    async def get_company_statistics(self, company_id: str) -> CompanyStatistics:
        ...
        try:
            ...
            company_age = (utc_time() - company.created_at).days if company.created_at else None
    ...
    async def _calculate_recent_application_trend(self, applications: List[JobApplication]) -> str:
        ...
        recent_count = sum(1 for app in applications if app.applied_date >= last_7_days)
    ...
    def _calculate_score_distribution(self, ats_reports) -> Dict[str, int]:
    ...
    def _get_top_keywords(self, keyword_lists: List[List[str]], top_n: int = 5) -> List[str]:
    ...
    def _group_applications_by_date(self, applications: List[JobApplication]) -> Dict[date, int]:
        """Group applications by date"""
    ...
    def _calculate_velocity_and_direction(self, daily_applications: List[TrendDataPoint]) -> Tuple[str, str]:
    ...
    def _find_peak_application_days(self, applications: List[JobApplication]) -> List[str]:
        ...
        for app in applications:
            if app.applied_date:
                day_of_week = app.applied_date.weekday()  # 0 = Monday
    ...
    async def _get_industry_comparison(self, job: Job) -> Dict[str, float]:
    ...
    def _count_jobs_last_12_months(self, jobs: List[Job]) -> int:
    ...
    def _determine_hiring_activity_level(self, jobs_12_months: int, active_jobs: int) -> str:
    ...
    async def _get_cached_statistics(self, job_id: str) -> Optional[JobStatistics]:
        ...
        try:
            ...
            if cached_data:
                ...
                return JobStatistics(**data)
        except Exception as e:
            logger.warning(f"Error retrieving cached statistics: {e}")
        
        return None
    
    async def _cache_statistics(self, job_id: str, statistics: JobStatistics):
    ...
    def _get_fallback_application_stats(self, job) -> ApplicationStatistics:
    ...
    def _get_fallback_competitiveness_metrics(self) -> CompetitivenessMetrics:
    ...
    def _get_fallback_trend_analysis(self) -> TrendAnalysis:
    ...
    def _get_fallback_company_stats(self) -> CompanyStatistics:
    ...
    def _get_minimal_fallback_statistics(self, job_id: str) -> Optional[JobStatistics]:
