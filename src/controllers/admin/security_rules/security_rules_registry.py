...
JOBSEEKER_RULE_REGISTRY = {
    ...
    "Multiple accounts from same IP": lambda ctx: ctx.security_service.detect_multiple_accounts(ctx.jobseeker),
    "Unrealistic application timing": lambda ctx: ctx.security_service.detect_burst_applications(ctx.jobseeker),
    "Suspicious resume content": lambda ctx: ctx.security_service.resume_has_boilerplate(ctx.jobseeker.resume),
}
